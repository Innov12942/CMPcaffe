...
def generate_fc_layer(kernel_num, layer_name, bottom, top, filler="xavier", sparse_ratio=0, class_num=256, quantize_term="false"):
    tmp =''
    if filler == 'gaussian':
      tmp = '''    std: 0.01
	  '''
...
def generate_lenet(stage):
    ...
    if stage >= 3:
      ...
      ratio = 0
    network_str += generate_fc_layer(500,"fc1","pool2","fc1","xavier",ratio,32,quantize_term)
    network_str += generate_activation_layer("relu1", "fc1", "fc1", "ReLU")
    if stage >= 4:
      ratio = sparse_ratio_vec[3]
    else:
      ratio = 0
    network_str += generate_fc_layer(10,"fc2","fc1","fc2","xavier",ratio,32,quantize_term)
